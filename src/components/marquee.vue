<template>
  <div class="fs-mqrquee-component">
    <mu-divider class="border-1px"/>
    <ul class="inner-wrapper" ref="innerDom" :style="{width: innerWidth + 'px'}" :class="{show: canShow}">
      <li class="mqrquee-item">
          <span v-for="(item,index) in listArr">{{item.Str}}</span>
      </li>
    </ul>
    <mu-divider class="border-1px"/>
  </div>
